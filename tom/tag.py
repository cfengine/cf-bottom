...
class Tagger:
    ...
    def get_current_tag(self, repo):
        return repo.run_command(
            "describe", "--abbrev=0", capture_output=True
        ).stdout.rstrip()
    ...
    def get_next_build_tag(self, old_tag, branch):
        if "build" in old_tag:
            ...
            if m:
                ...
                raise TagParsingException(
                    "Could not split tag [%s] into parts" % old_tag
                )
        else:
    ...
    def suggest(self, branch):
        ...
        self.slack.reply(
            "to add *build* tag, say (%s will be created at the tip of %s branch):\ntag: %s,%s"
            % (next_build_tag, branch, branch, next_build_tag)
        )
        next_final_tag = self.get_next_final_tag(current_tag, branch)
        if next_final_tag:
            self.slack.reply(
                "to add *final* tag, say (%s will be created at existing %s tag):\ntag: %s,%s,%s"
    ...
    def run(self, args):
        ...
        if len(args) == 1:
            ...
            raise TooManyArgumentsException(
                "received %d arguments, expected max 3" % len(args)
            )
