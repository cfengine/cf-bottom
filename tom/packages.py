...
class PackageMapper():
    ...
    def collect_packages(self, url):
        ...
        if not release_request.ok:
            raise URLDownloadFailureException('failed to download %s, return code %d' % (url, release_request.status_code))
        try:
        ...
        return {item['package']: {'url': item['URL']}
                for table in release_data['artifacts'] for item in release_data['artifacts'][table]
