...
class ChangelogGenerator():
        ...
        for i in range(5):
            ...
            if m:
                return ('\n'.join(changelog_lines[:i]),
                        m.group(1),
                        '\n'.join(changelog_lines[i:]))
        raise ChangelogParsingException(('Failed to find version in changelog '+
    ...
    def get_changelog_for(self, name, arg, old_version, branch):
        ...
        try:
            ...
            cmd = ['../core/misc/changelog-generator/changelog-generator',
                    arg,
    ...
    def generate_changelog_in_repo(self, branch, repo):
        ...
        (header, old_version, old_changelog) = self.split_changelog_into_parts(changelog_filename, repo)
        if header != '':
            header += '\n'
        ...
        if name != 'enterprise':
            ...
            changelog_contents = header+new_version+':\n'+new_changelog+'\n'+old_changelog+'\n'
        else:
            # generate changelog for all enterprise repos plus buildscipts repo
            changelog_enterprise = self.get_changelog_for(name, '--enterprise', old_version, branch)
            changelog_buildscripts = self.get_changelog_for('buildscripts', '--repo', old_version, branch)
            changelog_contents = (new_version+':\n'+changelog_enterprise+'\n'+
        ...
        pr_text = self.github.create_pr(
            'cfengine/' + name, branch, self.username, new_branchname,
    ...
    def run(self, branch):
        ...
        # generate changelogs
        prs = (self.generate_changelog_in_repo(branch, repo)
               for repo in repos)
